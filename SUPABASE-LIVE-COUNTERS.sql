...
as $$
begin
  if p_visitor_id is null or length(p_visitor_id) < 8 or length(p_visitor_id) > 120 then
...
declare
  ...
  result jsonb;
begin
  -- Works even if the players table has not been created yet.
  if to_regclass('public.players') is not null then
    execute 'select count(*) from public.players' into registration_total;
  end if;

  select jsonb_build_object(
    'unique_visitors', count(*),
    'page_views', coalesce(sum(page_views), 0),
    'countries_reached', count(distinct country_code) filter (where country_code <> 'XX'),
