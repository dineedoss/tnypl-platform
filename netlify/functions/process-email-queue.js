...
exports.handler=async(event)=>{
 ...
 const templates={
  ...
  changes_requested:p=>({subject:"TNYPL Registration — Corrections Required",html:`<h2>Action required</h2><p>Please contact info@tnypl.in to complete the requested corrections.</p>`}),
 ...
 for(const item of items||[]){
  ...
  const response=await fetch("https://api.resend.com/emails",{method:"POST",headers:{"authorization":`Bearer ${resend}`,"content-type":"application/json"},body:JSON.stringify({from:process.env.EMAIL_FROM||"TNYPL <registrations@tnypl.in>",to:[item.recipient_email],reply_to:"info@tnypl.in",subject:mail.subject,html:mail.html+"<hr><p>TNYPL · tnypl.in · info@tnypl.in</p>"})});
